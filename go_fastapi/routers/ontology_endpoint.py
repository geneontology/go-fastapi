...
from ontobio.golr.golr_query import run_solr_on, run_solr_text_on, ESOLR, ESOLRDoc, replace
from biolink.ontology.ontology_manager import get_ontology
from ontobio.io.ontol_renderers import OboJsonGraphRenderer
from biolink.api.entityset.endpoints.slimmer import gene_to_uniprot_from_mygene
...
subgraph_params.add_argument('relation', action='append', default=['subClassOf', 'BFO:0000050'],
...
ribbon_parser.add_argument('ecodes', action='append',
                           help='List of Evidence Codes to include (e.g. EXP, IDA). Has priority over exclude_IBA')
ribbon_parser.add_argument('exclude_IBA', type=inputs.boolean, default=False, help='If true, excludes IBA annotations')
ribbon_parser.add_argument('exclude_PB', type=inputs.boolean, default=False,
                           help='If true, excludes direct annotations to protein binding')
ribbon_parser.add_argument('cross_aspect', type=inputs.boolean, default=False,
                           help='If true, can retrieve terms from other aspects if using a cross-aspect relationship such as regulates_closure')
    ...
    def get(self):
        ...
        for subject_id in subject_ids:
            ...
            for cat in categories:
                ...
                terms = self.get_category_terms(cat)
